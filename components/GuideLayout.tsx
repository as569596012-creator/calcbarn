...
import { getGuide, relatedCalculatorsForGuide } from "@/lib/guides";
...
export default function GuideLayout({
  ...
  const calculators = relatedCalculatorsForGuide(guide);
    ...
    <article className="mx-auto max-w-3xl px-4 py-10">
      ...
      {calculators.length > 0 && (
        <section className="mt-10 rounded-2xl border border-brand-200 bg-brand-50 p-6">
          <h2 className="text-xl font-bold text-gray-900">Skip the math — use the calculator</h2>
          <p className="mt-1 text-sm text-gray-600">
            Free, instant, and accurate. Results update as you type.
          </p>
            ...
            {calculators.map((c) => (
              ...
              >
                ...
                <span>
                  ...
                  <span className="block text-sm text-gray-600">{c.intro}</span>
                </span>
              </Link>
            ))}
          </div>
